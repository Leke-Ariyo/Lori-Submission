...
class CustomerRentalChargeView(APIView):
    def get(self, request, pk):
        def update_date(each):
            each.days_borrowed = (timezone.now() - each.date_logged).days
            each.save()
        customer_instance = Customer.objects.get(pk=pk)
        result = BookLending.objects.filter(customer=customer_instance)
        [update_date(obj) for obj in result]
        serializer = GetBookLendingSerializer(result, many=True)
        return Response({"total_fee":customer_instance.amount_due, "rented books":serializer.data})     